        ...
        <div class="card">
          ...
          <div class="card-body">
            <form action="{{route($routePrefix.'.list')}}" method="GET">
              ...
              <div class="form-group">
                <label>@lang('validation.attributes.product_category')</label>
                <select name="filters[product_category]"
                  class="form-control form-control-sm">
                  <option value=""></option>
                @foreach(App\Helpers\ApplicationConstant::PRODUCT_CATEGORY as $val => $label)
                  <option value="{{$val}}" {{ (string)$obj->product_category == $val ? 'selected' : '' }}>
                    @lang('application-constant.PRODUCT_CATEGORY.'.$label)
                  </option>
                @endforeach
                </select>
              </div>
